...
const logIn = async (conv) => {
  ...
  const employeeIdNumber = wordsToNumbers(conv.session.params.employeeIdNumber)
  const pin = wordsToNumbers(conv.session.params.pin)
  
  await database.collection("Employees").where('employeeIdNumber', '==', `${employeeIdNumber}`).get().then(employees => {
		if(employees.docs[0]){
...
//   if it's the first time logging in for the session
  if ( conv.session.params.pin ){
    ...
    conv.add(`Hello ${conv.session.params.firstName}. What would you like me to record?`);
  } else {
...
const roomVerification = async (conv) => {
  return await database.collection("Patients").where("roomNumber", "==", conv.session.params.roomNumber.toLowerCase()).get().then(patients => {
      if(!patients.docs[0]){
        conv.add("I couldn't find that room number. Please record this event manually. ");
        conv.scene.next.name = 'start';
        return false;
...
const recordMealConsumption = async (conv) => {
  await roomVerification(conv).then(async patient => {
      ...
      await conv.add(`Okay, I recorded that ${patient.firstName} ${patient.lastName} ate ${conv.session.params.percentEaten} of their ${conv.session.params.meal}. `);
      conv.scene.next.name = "start";
  	}
  });
};
...
const activityLog = async (conv) => {
  ...
  await database.collection("ActivityLog").where("", "==", `${conv.session.params.person}`).orderBy('timeStamp').get().then(activityLog => {
		activityLog.docs.forEach(doc => {
        hasActivityLog = true;
        if(doc.data().bowel){
          response += `${timeDifference(Date.now(),doc.data().timeStamp)}, ${doc.data().person} was 
          ${doc.data().continentIncontinent} in the bowel and it was ${doc.data().stoolConsistency}. `;
        } 
      	else if(doc.data().meal){
          response += `${timeDifference(Date.now(),doc.data().timeStamp)}, ${doc.data().person} ate 
...
const whenEat = async (conv) => {
    await roomVerification(conv).then(async patient => {
        if(patient){
              await database.collection("ActivityLog").where("roomNumber", "==", `${patient.roomNumber}`).where("hasMeal", "==", true).
                orderBy('timeStamp', 'desc').get().then(activityLog => {
                    if(activityLog.docs[0]){
                      	conv.add(`${patient.firstName} had ${activityLog.docs[0].data().meal} ${timeDifference(Date.now(),activityLog.docs[0].data().timeStamp)}. `);
                    } 
                    else {
...
const whenBowel = async (conv) => {
    await roomVerification(conv).then(async patient => {
        if(patient){
              await database.collection("ActivityLog").where("roomNumber", "==", `${patient.roomNumber}`).where("bowel", "==", "bowel").
                orderBy('timeStamp', 'desc').get().then(activityLog => {      
                    if(activityLog.docs[0]){
                      conv.add(`${patient.firstName} had a ${activityLog.docs[0].data().stoolConsistency}, ${activityLog.docs[0].data().continentIncontinent} bowel movement 
                      ${timeDifference(Date.now(),activityLog.docs[0].data().timeStamp)}. `);
                    } 
                    else {
                      conv.add(`${patient.firstName} has no records. `);
                    }
...
const tipsTransferring = async (conv) => {
    await roomVerification(conv).then(async patient => {
        if(patient){
              await database.collection("Patients").where("roomNumber", "==", `${patient.roomNumber}`).get().then(patient => {      
